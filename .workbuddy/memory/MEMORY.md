...
- **待做范围（按 2026-07-16 参谋方案；V2.22.0 已落地 P0-3，后续自 V2.22.x 推进）**：
  ...
  8. ✅ 离线文书库（V2.27.0 升级，用户从参谋新增建议清单选定）：「文书库」一级模块（src/pages/LegalForms.tsx + PLATFORM_NAV.top 入口 + manifest.json 驱动）。V2.26.0 初版 60 份 PDF（行政 50 + 刑事 10 最新修正版）；V2.27.0 升级：①分类由「行政/刑事」二元改为**数组多标签 + 通用类**（行政刑事通用 15 种：受案登记表/受案回执/接受证据清单/移送案件通知书/传唤证/询问讯问笔录/笔录/调取证据通知书·清单/鉴定聘请书/收缴追缴物品清单/收取保证金通知书·回执/退还保证金通知书/没收保证金决定书），行政刑事筛选通用文书同时出现；②全部 79 份提供 **PDF + Word 双格式下载**（行政 50 Word 由用户制式文书.doc→docx 按标题块忠实拆分，5 份漏拆的用 pypdf+python-docx 重建；刑事 10 Word 为官方 PDF 重建）；③刑事补充 19 份核心空白参考模板（按公安部 2020 版式样重建：立案决定书/不予立案通知书/拘留证/拘留通知书/延长拘留期限通知书/逮捕证/逮捕通知书/取保候审决定书·执行通知书/监视居住决定书·执行通知书/搜查证/通缉令/释放通知书/释放证明书/扣押决定书/查封决定书/鉴定意见通知书/撤销案件决定书/终止侦查决定书/移送起诉告知书）——**此 19 份为重建参考模板非官方扫描件**，其余约 60 种刑事专属文书(律师参与/技术侦查/执行/刑事通用)暂未覆盖，若用户后续提供整本《刑事法律文书式样》来源可像行政 doc 一样精确拆分替换。支持文书名/式样号搜索、预览(新窗口)、PDF/Word 下载。
