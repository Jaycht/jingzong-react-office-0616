...
import type { FieldDefinition, FieldType, NavDepartment, WorkModule } from './types';

const f = (id: string, label: string, type: FieldType = 'text', required = false, options?: string[], customOptionKey?: string, multiple?: boolean): FieldDefinition => ({ id, label, type, required, options, customOptionKey, multiple });

const module = (
  deptId: string, deptLabel: string, moduleId: string, moduleLabel: string,
  desc: string, tabLabels: string[],
): WorkModule => ({
  id: moduleId,
  label: moduleLabel,
  departmentId: deptId,
  ...
  tabs: tabLabels.map((label, idx) => ({
    id: ${moduleId}-tab-,
    label,
    fields: fieldsFor(moduleId, label),
  })),
});

const singleModule = (
  deptId: string, deptLabel: string, moduleId: string, moduleLabel: string,
  desc: string, tabLabel: string,
): WorkModule => ({
  id: moduleId,
  label: moduleLabel,
  departmentId: deptId,
  departmentLabel: deptLabel,
  description: desc,
  tabs: [{ id: ${moduleId}-main, label: tabLabel, fields: fieldsFor(moduleId, tabLabel) }],
});
...
export const DEPARTMENTS: NavDepartment[] = [
  {
    ...
    icon: Landmark,
    modules: [
      module('office', '大队办公室', 'office-finance-assets', '经费与资产', '经费报账、物资采购、固定资产和福利发放等财务管理。', ['经费报账', '党费管理', '公务往来', '福利发放', '物资采购', '固定资产']),
      module('office', '大队办公室', 'office-party-attendance', '党建与考勤', '三会一课、主题党日、组织生活、党员管理、考勤、值班加班等日常管理。', ['三会一课', '主题党日', '组织生活', '党员管理', '考勤登记', '值班加班']),
...
export const ICONS = {
  Archive, Banknote, BookOpenCheck, ClipboardCheck,
  FileSearch, FileText, FolderKanban, Handshake, ListChecks,
  LockKeyhole, Megaphone, MessageSquareText, WalletCards,
};
