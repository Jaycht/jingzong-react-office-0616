...
export const VERSION_MINOR = 53;
export const VERSION_PATCH = 0;

export const CHANGELOG: string[] = [
  // ===== V2.53.0 =====
  "V2.53.0 新增 - 典法查阅法条详情页右侧新增「阅读进度导轨」：长法条（尤其像刑法这种几百条的）阅读时右侧常驻一条竖向进度条，随滚动实时填充，上方按法条结构分布可点击的「章节点」（有编/章的法条以章为节点、扁平无章的长法条自动均匀取最多 40 个节点、条数少的每条一个节点，悬停显示条号），点节点平滑跳转到对应条文；进度条正下方附「回到顶部」按钮（点击平滑滚回页首），整体 sticky 吸顶、随正文滚动始终可见，深色模式同步适配",
  // ===== V2.52.0 =====
  "V2.52.0 修复 - 案件时间轴卡片整屏显示「未命名」、字段出现英文字符（objectName / responsible 等）：根因是展示层（时间轴 / 案件详情 / 全局搜索）各自维护手写 FIELD_LABELS 映射，法制室「考核管理」的 objectName / responsible / baseScore / deductItem 等从未登记，于是把字段 id 原样当标签显示；标题推导只认有限字段链、不含 objectName / docTitle / lifeName。现新增 src/utils/fieldIndex.ts 从 moduleConfig 字段定义统一派生「字段 id → 中文标签」，并扩充 TITLE_FIELD_CHAIN 接「模块字段定义里第一个有值文本字段」兜底，任何模块新增字段自动带中文标签，不再出现英文 id；三处展示层全部改用 fieldIndex 解析（补 4 条单测锁定：中文标签解析、标题推导、空记录兜底）",
  "V2.52.0 修复 - 各模块列表页导出的 Excel 数据重复：根因旧 flattenRecord 对有可重复段（如涉案主体统计）的模块按「第一段的明细条数」展开成 N 行、主字段重复 N 次，且回导时每行被当成独立记录、3 条记录回导变 6 条。现改为「1 条记录 = 1 行」，可重复段按「段名 | 字段名 | 序号」编号列横向展开（如「涉案主体统计 | 公司名称 | 1」），导出再导入可无损还原，彻底消除重复（补 3 条单测锁定：1 行导出、完全相同记录自动跳过、编号列逆向还原数组）",
  "V2.52.0 新增 - 导出文件名追加到秒的时间戳，避免同日多次导出重名：文件名由「模块名 + 日期」改为「模块名_本地日期_时-分-秒」（yyyy-MM-dd_HH-mm-ss，按本地时区，修复旧 toISOString 的 UTC 偏差导致整体差一天的隐患），所有导出入口（模块 / 全量 / 选中 / 调证台账 / 操作日志 / CSV / 空白模板）统一采用",
  "V2.52.0 新增 - 「导出空白模板快捷录入」功能与说明：每个模块导出时若无数据即得到空白模板（表头已含可重复段编号列），用户在 Excel 填好再导入即成为便捷录入方式；导入导出页新增说明卡片（三步流程：导出空白模板 → Excel 填写 → 导入生成记录）并提示「完全相同的行自动跳过、改后导入为追加」，模块页导出按钮加提示文案、空白模板额外附带一张「填写说明」sheet 供首次使用参考",
  // ===== V2.51.0 =====
  "V2.51.0 修复 - 文书库与典法查阅点「上传」时会同时弹出「上传弹窗 + 管理页面」两个窗口（上传弹窗叠在前，管理页在后面，看着像是两个窗一起冒出来）：根因是「上传」与「管理」共用面板，点「上传」先把管理页面渲染出来、再在其上叠一层上传表单；现改为两种入口互斥打开 —— 点「上传」只见上传表单（存完即回到列表页），点「管理」才见管理页面（条目列表 + 回收站），管理页内的「上传新条目」仍可在其上新增，不再出现两个窗口同时弹出",
  "V2.51.0 新增 - 文书库上传弹窗的「分类」改为下拉式多选菜单并支持自定义：此前分类是有限的固定几项，收集不全或分类有更新时无处安放；现改为下拉选择、可同时选多个分类（文书常见一份文书跨多个分类），下拉列表底部保留「自定义添加分类」输入框，输入后回车或点「添加」即写入候选项并自动勾选，自定义的分类与内置分类一样长期保存、下次打开仍在（法条仍为单选，一个法条归一个分类）",
  "V2.51.0 修复 - 调证登记「从平台粘贴导入」不再覆盖已有记录，改为「只追加 + 按申请单号刷新」：① 不同申请单号一律追加为新记录，即使案件（线索）名称、编号完全相同也不合并 —— 一个案件多次调证是常态，合并会把多份调证挤成一条、丢掉先前记录；② 申请单号是唯一键，同一申请单号再次导入时只更新「调证状态、反馈时间、查控结果」三项并以本次平台数据为准，申请人、申请事由、备注、协查单位等手填内容一概保留，绝不整条覆盖；③ 解析预览的新增「导入方式」列逐条标出「新增 / 更新」，勾选框文案同步显示本次将新增多少条、更新多少条，导入前即可核对",
  // ===== V2.50.0 =====
  "V2.50.0 修复 - 两项实测问题：① 一键生成演示数据后瞬间弹出一屏到期预警 —— 演示记录参与了法律时限预警计算，现预警口径统一排除演示数据（三处一致：预警弹窗服务、工作台待办预警、到期预警组件），演示数据只服务于功能演示，不再制造待办；② 演示标记漏到界面上（案件时间轴 / 案件详情是「遍历 data 的所有键」来展示字段的，于是内部标记被当成业务字段列出，两个下划线在界面里渲染成一道长横线，看起来是「—demo：v1」）—— 标记改存记录顶层 MassRecord.demo，不再写进 data，展示层同时跳过一切 __ 前缀的内部键（迁移遗留的 __oldCaseId 同源问题一并解决），旧版已生成的演示数据仍可识别与清除",
  "V2.50.0 新增 - 三项能力补齐：① 法律时限预警弹窗改为两档处置 —— 「下次登录提醒」（本次登录不再打扰，下次打开软件重新提醒）与「不再提醒」（该条预警不再弹出，工作台「到期预警」中仍可查看）；此前预警弹窗只有「我知道了」，且根本没把预警标识传给通知窗口，点任何按钮都不会记住，下一次轮询照样弹；② 一键演示数据补上「可多次添加」的明细段 —— 嫌疑人信息、涉众主体、报案人、调证清单这类可重复段此前一条都没生成（并且「字段全部落在可重复段里」的页签整个被跳过），没法测试新增 / 逐条删除 / 折叠展开；现在每个可重复段按字段定义生成 2 条明细，存法与抽屉保存完全一致（明细只进数组，不写记录主表）；③ 文书库与典法查阅把「上传」从「管理」里拆出来 —— 页面头部拆成「上传」与「管理」两个入口，上传新条目不需要密码（收集毕竟不全或有更新，随时可补），只有编辑、删除、回收站恢复 / 彻底删除 / 清空才按动作弹密码框，验证一次后在本次面板内保持解锁",
  "V2.50.0 优化 - 三项体验调整：① 演示数据全面脱敏 —— 人员姓名一律「姓 + 某某」，案件名称中的当事人同样以某某替代，公司名称去掉真实品牌与字号只保留行业属性，设备型号不再出现具体品牌机型（演示数据里不再出现真实姓名与真实企业名）；② 案件图谱默认使用全屏画布 —— 原默认尺寸装不下节点较多的图谱，打开时看起来「显示不完整」，需要手动点全屏才能看全；③ 工作台移除「串并案线索」展示模块（串并案识别与完整分析仍在「串并案」页，工作台不再重复展示）",
  // ===== V2.49.0 =====
  "V2.49.0 新增 - ① 关闭程序确认改为应用内弹窗：原先由主进程 dialog.showMessageBoxSync 弹出 Windows 原生对话框，样式与软件整体风格不符，且只有「最小化到托盘 / 退出软件」两个按钮、误点后无法反悔；现改由渲染进程用与软件一致的面板呈现，并新增「取消」——取消后窗口保持打开，关闭行为设置（每次询问 / 直接退出 / 最小化到托盘）仍由主进程持有；② 文书库与典法查阅支持自行补充与修订（收集毕竟不全或有更新）：两个库右上角均新增「管理」入口，凭管理员密码（初始 JDZZ3231268，可在面板内修改）可手动上传新增文书（PDF，Word 可选）与法条（txt）、按名称/分类搜索、编辑任意条目的名称与分类式样、删除条目；③ 删除进回收站：删除的条目先移入回收站，可随时恢复到原有位置，回收站支持单条彻底删除与一键清空，内置条目的「删除」是隐藏而非销毁（恢复即重新出现），自定义条目彻底删除时才清理其磁盘文件；④ 一键生成演示数据：系统设置 → 数据管理新增面板，可按真实字段定义一次为全部 5 个科室 / 23 个模块 / 44 个页签生成贴合业务的演示条目（日期落在近三个月、金额/人名/单位/编号按字段语义取值，调证登记另按台账口径生成申请单），并附带几篇日常随手记；演示条目带标记、可一键清除，与真实数据互不影响，便于测试与演示而无需逐条手工录入",
  "V2.49.0 修复 - 卸载时选择「同时删除所有用户数据」实际不生效（重新安装后用户名与工作记录依然在）：根因是 perMachine 安装（nsis.perMachine = true）下卸载器全程处于 SetShellVarContext all 状态，此时 $APPDATA 指向 C:\\ProgramData 而非当前用户的 AppData，原来那句删除操作删的是一个根本不存在的目录，所以选定「是」也等于没删；现显式切回 current 上下文再执行清理，范围覆盖 %APPDATA%\\jingzong-work-log（IndexedDB / Local Storage / 登录用户名 / 路径配置）、更新器缓存 %LOCALAPPDATA%\\jingzong-work-log-updater 以及各盘 jingzong_data（附件与文书库上传件）。注意：卸载逻辑随安装包编译，本修复对本次及之后构建的安装包生效——需先用新版安装包覆盖安装一次，其内置的新卸载器才会在下次卸载时按正确路径清理",
  "V2.49.0 优化 - 文书库与典法查阅的自定义条目与内置条目统一渲染：页面看到的清单均为「内置清单 + 本地覆盖层」的合并结果，编辑内置条目只记录本地覆盖、删除内置条目只作隐藏，内置清单文件与构建产物始终保持只读；自定义条目上传的文件由主进程落到数据目录（jingzong_data/legal），预览用系统默认程序打开、下载经主进程读出内容后另存，因此任何增删改都不会破坏随包发行的官方文书与法条",
  // ===== V2.48.0 =====
  "V2.48.0 新增 - 调证登记新增独立「调证状态」字段，「备注」恢复为自由文本框：① 「反馈结果」段新增「调证状态」下拉（已提交 / 已审批 / 已发送 / 已反馈，下拉底部保留「自定义添加」），状态不再寄居在备注里；② 备注改回空白多行文本框，用来写真正的备注；③ 列表「备注」列展示与编辑的仍是调证状态（列名与列序未变，仍是十一列之一），存的是新字段 requestStatus —— 旧数据（状态塞在备注的「申请状态：X；报文状态：Y」标注串里）在打开抽屉或点格编辑时自动迁移，用户手写的自由备注不会被当成状态吞掉，改状态时也会顺手清掉备注里的旧标注串；④ 从平台粘贴导入改为写 requestStatus 字段、备注留空，预览表格新增的「调证状态」列可直接核对；⑤ 抽屉「反馈结果」页填写反馈信息（反馈成功数 / 失败数，或填了反馈时间）后，调证状态自动置「已反馈」，且只在本次确实改动过反馈字段时才触发；⑥ 调证登记不参与通用的「状态徽标 / 状态筛选 / 已办结 KPI」，避免凭空多出「办理中 / 待补充 / 已办结」三个与调证口径不搭的筛选项",
  // ===== V2.47.0 =====
  "V2.47.0 新增 - 调证登记「备注」列改为「调证状态」单档呈现：① 平台给的「申请状态：已审批；报文状态：已反馈」其实是同一条推进状态，现折叠成四档之一（已提交 / 已审批 / 已发送 / 已反馈）——报文状态为「已发送 / 已反馈」时以报文状态为准，否则采用申请状态（未审批 / 待审批 / 已提交 → 已提交，已审批 → 已审批，「已接收」归入已发送）；② 列表「备注」列只显示这一档，点击即在四档下拉里切换（下拉底部保留「自定义添加」，发送失败 / 审批拒绝这类异常状态与自由备注仍写得进去），旧数据的标注串在打开抽屉时自动归一，列表里点一下那一格即完成落库迁移；③ 从平台粘贴导入改为按同一规则折叠成单档入库，预览表格新增「调证状态」列供导入前核对，案件校验状态不再塞进备注；④ 抽屉「反馈结果」页填写了反馈信息（反馈成功数 / 失败数，或填了反馈时间）后，备注自动置为「已反馈」——且只在本次确实改动过反馈字段时才自动改写，编辑旧记录随手保存不会把平台带来的「已发送」冲掉，用户手写的自由备注（如「微信支付宝调取」）也不被覆盖",
  // ===== V2.46.0 =====
  "V2.46.0 新增 - 粘贴导入的调证案件信息与全项目打通：此前只有「抽屉表单保存」和「列表行内编辑」会重建全局索引，平台粘贴导入与 Excel 导入属于绕过表单的批量写入，导入的案件名称/编号既不进全局案件索引、也不进输入历史池 —— 别的模块的案件名称/编号下拉看不到、选不中，更谈不上「选中后自动填充主办民警/受案日期」，只有重启（App 启动时重建索引）才补上。现两条批量导入路径写完统一收尾：① 新增 massStore.rebuildGlobalIndexes()，从全量记录一次重算「案件/线索 ⇄ 编号」与「嫌疑人」两个联动池并广播数据变更事件，粘贴导入、Excel 导入完成后各调用一次，导入数据与手工录入完全同等对待；② 导入值同步并入全局输入历史池（申请单号 / 查控类型 / 申请时间 / 案件名称 / 案件编号），导入过的业务键在其它模块可联想复用；③ 全局案件索引扩展支持线索字段：clueName / clueNo 此前永远进不了索引（重建时只读 caseName / caseNo），「线索调证」记录的名称与编号在其它模块完全联想不到，现与案件字段同池、互为回退；④ 列表行内编辑把改动值一并写入历史池，线索名称/编号这类只认历史池的字段改完也能全项目联想；⑤ 输入历史池批量写入收敛为 recordFieldValues()，一次读取一次落盘，替代逐字段各自落盘",
  // ===== V2.45.0 =====
  "V2.45.0 新增 - 调证登记列表按录入习惯重定表头，导出模板保持不变：① 列表改为十一列「序号 / 案件（线索）名称 / 线索\\案件编号 / 申请时间 / 申请单位 / 申请人 / 申请单号 / 反馈时间 / 查控结果 / 请求查控人 / 备注」，新增「线索\\案件编号」（案件取 caseNo、线索取 clueNo，可点格直接填）与「申请单位」（复用协查单位下拉，弹层底部保留下拉自定义添加，与抽屉共用同一份自定义选项）；② 列表与导出彻底解耦：导出仍按单位《资金查控情况登记台账（周五报送）》十列（含「申请事由」）一字不变，改列表不再影响导出台账；③ 行内编辑由「点整行改整行」改为「点哪一格改哪一格」，只有被点中的单元格变控件，其余列保持只读，减少误改；④ 「查控结果」编辑时三项数字框各自带「未反馈 / 成功 / 失败」小标题，不再让用户猜哪个是哪个；⑤ 反馈时间点一下就自动填入当天（可再改成实际日期），粘贴导入的反馈时间默认取登记当天；⑥ 修复日期存储时区错位：日期字段改存本地日期串（yyyy-MM-dd），此前经 DatePicker 录入的日期按 UTC 存储会导致列表显示与台账导出整体差一天，「申请时间 / 反馈时间」导出格式与单位模板一致",
  // ===== V2.44.0 =====
  "V2.44.0 新增 - 列表行内编辑：点击列表中的某一行即就地进入修改（不再弹出「案件 360° 查看详情」全屏视图，改一个字段不必再走「点行→查看→编辑→抽屉」四步）：① 文本/多行文本直接敲、日期点击弹出日期选择器、下拉字段点开选项、数字用数字框，「查控结果」列拆成未反馈/成功/失败三个小数字框；② 回车或 ✓ 保存、Esc 或 ✗ 放弃，切换或点击另一行时自动提交上一行（等同表格失焦提交），编辑行整行高亮并在左侧显示色条；③ 「操作」列的 查看 / 编辑 / 删除 按钮全部保留，抽屉仍负责附件等复杂字段；④ 卡片视图点击卡片改为直接打开编辑抽屉（卡片无行内编辑概念），卡片内查看按钮仍可打开详情",
  "V2.44.0 优化 - 台账列口径与行内编辑合流到单一事实源 utils/requestLedger.ts：列定义（标题/宽度/取值/可编辑字段）统一由 LEDGER_COLUMNS 提供，列表、导出、粘贴导入三处共用；行内编辑写回时自动区分案件/线索记录（线索调证写 clueName，不污染 caseName），并兼容旧版 requestItems 数据（编辑后自动扁平化，清空字段不会被旧值顶回来，多条申请单仍保留）；「反馈时间」取值口径同步统一为「扁平字段优先、旧段回退」",
  "V2.44.0 修复 - 从资金查控平台粘贴导入后「反馈时间」整列为空，导致导出台账该列无内容：现默认取登记时间（本条导入落库的时刻），之后可在列表点行就地改成真实反馈日期",
  // ===== V2.43.0 =====
  "V2.43.0 优化 - 调证登记全面对齐《资金查控情况登记台账（周五报送）》，做到「列表所见即导出所得」：① 列表列由原来的「编号/名称/来源/类型/协查单位/调证对象」改为台账十列（序号/案件（线索）名称/申请时间/申请事由/申请人/申请单号/反馈时间/查控结果/请求查控人/备注），列名与顺序与单位台账模板逐字一致，导出的表直接可用、无需再编辑列；② 申请信息由可重复段 requestItems 扁平化为记录主字段（一条记录 = 一份申请单），新增「申请事由 / 申请人 / 请求查控人 / 备注」四个字段，新建表单按「案件（线索）信息 → 申请信息 → 反馈结果 → 协查信息」四步组织；③ 新增 utils/requestLedger.ts 作为台账列名与取值规则的单一事实源，列表、导出、粘贴导入三处共用，避免口径漂移，并兼容旧版 requestItems 数据（读取自动回退首条，编辑旧记录时自动迁移到扁平字段，不丢数据）",
  "V2.43.0 修复 - 台账导出三列内容与模板不符：① 「申请事由」原错填「查控类型」（常规查询/冻结申请…），而单位台账该列填的是「刑警专案查询 / 政保查询 / 某某投资人查询」这类自由文本，现改为取新增的 applyReason 字段；② 「申请人」「请求查控人」原先硬编码空串、永远为空，现分别由 applicant / requester 字段提供，可在表单补录；③ 「备注」原先读取 data.remarks，但该字段从未在字段定义中出现（仅粘贴导入时临时塞值），现正式定义备注字段，导出读值不再恒空",
  "V2.42.0 新增 - 调证登记支持「从资金查控平台粘贴导入」并新增单位版台账导出：① 新增 src/utils/platformPasteParser.ts，把平台复制出来的表格文本（TSV 多行错位与 HTML 折叠两种剪贴板形态）解析为结构化申请单行，自动识别案件编号/案件名称/查控类型/任务·未反馈·反馈成功·反馈失败数/申请单号/创建时间；② 新增「从平台粘贴导入（资金查控平台）」弹窗（新建弹窗蓝头按钮，仅调证登记模块）——粘贴→解析→勾选预览→按申请单号自动查重→批量入库，支持预设协查单位与调证平台；③ 调证信息段新增「查控类型」字段（常规查询/冻结申请/继续冻结申请/解除冻结申请），反馈结果段新增「未反馈数」字段；④ 新增 exportRequestLedger()，按单位《资金查控情况登记台账（周五报送）》模板导出十列（序号/案件（线索）名称/申请时间/申请事由/申请人/申请单号/反馈时间/查控结果/请求查控人/备注，标题合并 A1:J1，按申请单展开，查控结果自动拼「未反馈x 成功y 失败z」），入口在「导入导出」页",
  "V2.42.0 构建 - 安装包体积 185.5MB → 97.7MB：① build.files 排除 dist/win-unpacked 与 dist/*.7z 等自包含路径——此前 electron-builder 输出目录 dist/win-unpacked 与 files 通配 dist/**/* 相互重叠，且 asar 是在 Electron 已被复制进该目录之后才构建，导致整个 Electron 运行时被二次打入 app.asar（asar 膨胀至 340MB）；② 14 个已由 Vite 完全打包进 dist/assets 的 UI 依赖从 dependencies 移至 devDependencies（主进程运行期只 require electron 与 path/fs/fs.promises 内置模块），asar 内 node_modules 条目由 18,461 归零",
  "V2.42.0 清理 - 修复 npm run typecheck 长期空跑（脚本缺 -p，作用在 files:[] 的 solution 式根 tsconfig 上恒退出 0、实为无类型门禁）并清零全仓 24 处类型错误：session.test.ts 缺 vitest 显式导入 13 处、electron.d.ts 漏声明 getAttachmentsPath 2 处（preload 早已暴露，纯声明缺口）、LegalKnowledge.tsx 死比较 view === 'ec77' 1 处、未使用声明 8 处；vitest 用例 73 条全过",
  "V2.41.22 清理 - 移除自动更新功能（项目定位为纯单机离线工具，不应具备联网检查/下载/安装更新的能力，与离线定位冲突）：删除 electron/main.cjs 的 electron-updater 加载块与 check-for-updates/download-update/install-update 三个 IPC 处理器及 update-available/update-progress/update-downloaded 事件转发；preload.cjs 移除对应的 checkForUpdates/downloadUpdate/installUpdate/onUpdateAvailable/onUpdateProgress/onUpdateDownloaded 六个方法；electron.d.ts 移除 checkForUpdates/downloadUpdate/installUpdate 类型声明；package.json 移除 electron-updater 依赖。经核查 src 无任何 UI 入口调用该能力，属惰性代码，移除零功能损失",
