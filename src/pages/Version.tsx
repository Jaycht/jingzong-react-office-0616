      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        {CHANGELOG.map((c, i) => (
          <motion.div key={c.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.08 }}
            ...
            <div><div style={{ fontSize: 11.5, color: "#6B7280" }}>{c.label}</div><div style={{ fontSize: 14, fontWeight: 700 }}>{"历史累计"}</div></div>
          </motion.div>
        ))}
      </div>
        ...
        <div>
          {allUpdates.map((u, i) => (
            <motion.div key={u.version} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 + i * 0.08 }}
              ...
              {u.items.length > 0 && u.items.map(item => (
